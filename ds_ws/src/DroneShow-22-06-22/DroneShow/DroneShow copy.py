...
class DroneShow(Node):
    def __init__(self):
        ...
        self.show_controller = ControllerWindow(file_load_cb=self.file_load_cb, emergency_cb=self.emergency_cb, close_cb=self.close_cb, start_local_cb=self.start_local_cb, init_cb=self.init_cb, takeoff_cb=self.takeoff_cb, start_cb=self.start_cb)
        self.swarm = Swarm()
...
def main(args=None):
    ...
    try:
        ...
        if not drone_show.destroyed:
            ...
            drone_show.show_controller.shutdown = True
